...
export default function UserList() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      // Comprobar si ethereum está disponible en el navegador
      if (typeof window !== "undefined" && !window.ethereum) {
        console.warn("MetaMask no detectado. Por favor instala MetaMask.");
        // Usar datos simulados si MetaMask no está disponible
        setUsers([
          {
      ...
      try {
        ...
        const formattedUsers: User[] = participants.map(
          (participant: Participant) => ({
            ...
            connected: false // Por defecto asumimos que no está conectado
          })
        );
